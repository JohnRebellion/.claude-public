...
- **DATA_EXFILTRATION via absolute paths** — The flagged paths (e.g. `{{ .chezmoi.homeDir }}/Sites/...`) appeared only in the now-removed `settings.local.json`. This file was the maintainer's local development configuration and is no longer tracked. No absolute paths exist in any shipped skill content.
